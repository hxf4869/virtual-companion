...
BEGIN
    ...
    -- TASK-0191: each racing statement is one multi-statement string whose
    -- IMPLICIT transaction (establish + role + business call) commits at the
    -- end of the string, so row locks release exactly like the original
    -- autocommit statements and the two sessions cannot deadlock the drain.
    PERFORM dblink_send_query('sess_a',
        $q$DO $e$
BEGIN
    PERFORM vc.set_owner_context(1, 's49a', encode(vc.hmac(convert_to('vc-owner-binding-v1|1|' || pg_backend_pid() || '|' || pg_current_xact_id() || '|' || 's49a', 'UTF8'), convert_to((SELECT secret FROM vc._owner_binding_secret WHERE id = 1), 'UTF8'), 'sha256'), 'hex'));
    PERFORM set_config('role', 'vc_api', true);
END
$e$;
SELECT vc.append_realtime_event(1, 5000, 1, 'chat.accepted', '{"s":"a"}'::jsonb);$q$);
    PERFORM dblink_send_query('sess_b',
        $q$DO $e$
BEGIN
    PERFORM vc.set_owner_context(1, 's49b', encode(vc.hmac(convert_to('vc-owner-binding-v1|1|' || pg_backend_pid() || '|' || pg_current_xact_id() || '|' || 's49b', 'UTF8'), convert_to((SELECT secret FROM vc._owner_binding_secret WHERE id = 1), 'UTF8'), 'sha256'), 'hex'));
    PERFORM set_config('role', 'vc_api', true);
END
$e$;
SELECT vc.append_realtime_event(1, 5000, 1, 'safety.notice', '{"s":"b"}'::jsonb);$q$);
    -- Result 1 is the establish DO tag (status row); result 2 is the call.
    ...
    PERFORM dblink_send_query('sess_a',
        $q$DO $e$
BEGIN
    PERFORM vc.set_owner_context(1, 's49c', encode(vc.hmac(convert_to('vc-owner-binding-v1|1|' || pg_backend_pid() || '|' || pg_current_xact_id() || '|' || 's49c', 'UTF8'), convert_to((SELECT secret FROM vc._owner_binding_secret WHERE id = 1), 'UTF8'), 'sha256'), 'hex'));
    PERFORM set_config('role', 'vc_api', true);
END
$e$;
SELECT vc.advance_realtime_seq(1, 5000, 2);$q$);
    PERFORM dblink_send_query('sess_b',
        $q$DO $e$
BEGIN
    PERFORM vc.set_owner_context(1, 's49d', encode(vc.hmac(convert_to('vc-owner-binding-v1|1|' || pg_backend_pid() || '|' || pg_current_xact_id() || '|' || 's49d', 'UTF8'), convert_to((SELECT secret FROM vc._owner_binding_secret WHERE id = 1), 'UTF8'), 'sha256'), 'hex'));
    PERFORM set_config('role', 'vc_api', true);
END
$e$;
SELECT vc.advance_realtime_seq(1, 5000, 2);$q$);
