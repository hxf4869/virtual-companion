...
20. **COMP-CFG 角色结构化配置**（FR-COMP-003）：✅ 已交付（V47）——
    catalog companion-prefs + PATCH /relationships/{id} + 组装器批准片段 +
    前端「角色设置」页。名称只作标签，禁止自由 Prompt。
...
- **FR-DATA-001** 账号信息/模型说明汇总查看。
- **FR-RES-002** 静态安全求助入口。
- **A4** 隐私/AI 说明页。
- **QUOTA-MIN**（A3-003）仍为 Owner 冻结，动手前必须确认解冻边界。
