 ...
 * rejected: the token, the current fence, the lease or the tenant context did
 * not all match. Callers must treat {@code 0} as fail-closed and never retry a
 * late write (INV-WORKER-001).
 */
public class WorkItemClaimService {
    ...
    public List<WorkItemClaim> claim(long ownerUserId, String fence, int leaseSeconds, int limit) {
        return jdbc.query(
                "SELECT owner_user_id, id, kind, ref_id, payload, claim_token "
