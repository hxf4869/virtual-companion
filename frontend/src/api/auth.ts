// TASK-0034: identity API client (login / refresh / logout). The transport is
// injected so the store and specs can mock request() exactly like
...
// in a URL or a log; only the auth store persists tokens, and it never writes
// them into chat drafts or memory content.
...
function asTokens(json: unknown): AuthTokens | null {
  ...
  const refreshToken = asString(o, "refreshToken");
  const accountId = asString(o, "accountId");
  const role = asString(o, "role");
  const expiresInSeconds = Number(o.expiresInSeconds);
  if (!accessToken || !refreshToken || !accountId || !role || !Number.isFinite(expiresInSeconds)) {
    return null;
  }
  return {
    accessToken,
...
/** Renew a session. Existence-hidden: non-OK -> null; transport failures propagate. */
export async function refresh(
  t: AuthTransport,
  refreshToken: string,
): Promise<AuthTokens | null> {
  const r = await t.request("POST", `${AUTH_BASE}/refresh`, { refreshToken });
  return r.ok ? asTokens(r.json) : null;
}

/** Revoke a refresh session. true only on an HTTP OK (idempotent). */
export async function logout(t: AuthTransport, refreshToken: string): Promise<boolean> {
  const r = await t.request("POST", `${AUTH_BASE}/logout`, { refreshToken });
  return r.ok;
}
