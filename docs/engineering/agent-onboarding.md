...
```text
AGENTS.md
  -> .harness/project-state.yaml + append-only task-ledger.yaml + task-backlog.yaml
...
3. 根据摘要读取 `.harness/project-state.yaml`、`.harness/task-ledger.yaml`、`.harness/task-backlog.yaml`、
   活动任务、Context Lock 和精确 Skill。
