...
5. Context Lock 使用 Base Commit 中的仓库相对路径；历史外部资料只能通过仓库别名或不可复验的 provenance 明示保留。
6. `doctor.py` 自动执行任务发现、Context、Skill、保护路径、审批、Diff Scope、Schema、状态和入口一致性检查。
7. `.harness/commands.yaml` 是命令注册表；`precheck.py` 用当前 Python 解释器执行，PowerShell/POSIX 文件只做薄包装，CI 调用同一入口。
8. Harness 自身属于 C4：必须有人工批准、独立复核和失败场景测试，不得为当前任务放宽自身。
