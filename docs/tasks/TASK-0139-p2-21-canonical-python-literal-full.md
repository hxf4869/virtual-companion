# TASK-0139：P2-21 canonical Python 字面量 python 合同（含 Agent 入口与恢复记录断言修复）
...
authorizationCommit: c40ca1c90b722dbbf62626ab915fe2fa1c59dc52
contextFingerprint: 5a8a659239cabc423a4b69df9ad9164655bb2c1b78aa85cc02bc15091ea3751b
contextLock: docs/tasks/context/TASK-0139.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C4
  surfaceId: TASK_0139_P2_21_CANONICAL_PYTHON_LITERAL_FULL
  ...
  indivisibleReason: >-
    canonical Python 解释器合同必须在 AGENTS、agent-entrypoints contentSha256 绑定、wrapper 与测试中
    同步收敛为单一字面量；AGENTS.md 变更还必然触发 TASK-0066/0067 恢复记录测试对 AGENTS.md
    不可变的既有断言（TASK-0138 已验证该断言是测试内绑定、非 Doctor/机器真源），因此本卡必须同时
    授权 agent-entrypoints.yaml 写路径与这两个测试断言的链内绑定修复。Owner 已于 2026-08-10 明确
    选择字面量 python；TASK-0137/0138 因写路径与断言范围缺失如实 REJECTED，本卡自 DRAFT 起修正。
...
readAllowlist:
  ...
  - docs/evidence/TASK-0138/evidence-pack.json
  - docs/evidence/TASK-0138/pre-closure-request.json
  - docs/evidence/TASK-0138/review-r1.md
...
humanApprovals:
  - scope: task-assignment
    ...
    evidence: >-
      ...
      因遗漏 agent-entrypoints 写路径、TASK-0138 因 TASK-0066/0067 恢复记录测试的 AGENTS.md
      不可变断言（测试内绑定，非机器真源）而如实 REJECTED 后，Owner 批准创建本 C4 卡自 DRAFT 起
      一并纳入两项修正并重跑完整门禁；禁止历史改写和伪造 PASS。
  - scope: harness-change
    ...
    evidence: >-
      ...
      与对应 wrapper 测试统一收敛为字面量 python；并将 TASK-0066/0067 恢复记录测试对 AGENTS.md
      的断言从「与当前工作树一致」改为「绑定链内固定历史对象」（Base 与 terminal 之间一致），
      不删测、不加 skip；不授权通用解释器 alias、环境变量 override、history rewrite、policy/Skill/
      阈值弱化或对其他任务复用。
...
> 2026-08-10 选择字面量 python 合同；TASK-0137/0138 因范围缺失如实 REJECTED，本卡修正承接。
...
（R1/R2 PASS），但唯一 Precheck 发现 AGENTS.md 变更导致 `.harness/agent-entrypoints.yaml`
contentSha256 drift（该文件被 READY 冻结 forbiddenPaths 禁止），如实 REJECTED。TASK-0138 从
DRAFT 起纳入该写路径并同步 contentSha256（Precheck PASS），但唯一完整 Harness 又发现
`test_task0066_replacement_is_exact_and_atomic` 与 `test_task0067_replacement_is_exact_and_atomic`
断言 TASK-0066/0067 Base 快照的 AGENTS.md 与当前工作树 byte-for-byte 相同——这两个恢复记录测试
假定 AGENTS.md 自其 Base 起不可变，被 P2-21 合法 AGENTS.md 变更首次打破（该断言是测试内绑定，
Doctor 生产与 ci-execution-policy recoveryInput 均不绑定 AGENTS.md），如实 REJECTED。
...
TASK-0066/0067 恢复记录测试断言改为绑定链内固定历史对象（Base 与 terminal 之间 AGENTS.md 一致），
完整 Harness 恢复绿线；跨平台 wrapper 仍调用同一 Python Harness 实现。
...
- scripts/harness/tests/test_harness.py：
  - wrapper 测试更新以匹配新合同（python3 不再回退，缺少 python 时失败关闭；合并为单测试方法
    保留既有平台门控，net 无新增 skip）；
  - `test_task0066_replacement_is_exact_and_atomic` 与 `test_task0067_replacement_is_exact_and_atomic`
    的 AGENTS.md 断言从「与当前工作树一致」改为「链内固定历史对象一致」（TASK_0066_BASE_COMMIT
    与 TASK_0067_BASE_COMMIT/TERMINAL_COMMIT 之间 byte-for-byte 一致），保留其余断言。
...
- 不修改 TASK-0066/0067 的 Card/Context/Evidence/Handoff/Ledger 条目或任何历史 Git 对象。
- 不修改历史 Evidence/Handoff（其中历史命令含 python3 属不可变历史，不追溯改写）。
- 不修 P2-23 至 P2-27、OpenAPI、业务、数据库、frontend 或 Provider DNS。

## 输入和前置条件

- Base `1a151e06d7d424b8fc9fadd57f32ffb11eb23067` 是 TASK-0138 单父 REJECTED terminal（实现已
  回滚为 Base 内容），已 push、fetch、`HEAD...origin/main=0/0` 且工作树 clean。
- TASK-0137/0138 候选提交保留字面量 python 实现历史供参考，本卡重新实现，不复用其门禁 PASS。
- TASK-0066 terminal = `b48984d1fbb61f809711f936066610576ad9426f`（= TASK-0067 Base），
  TASK-0067 terminal = `0be905ab953975476802a5f1e0eef68b978635d2`；两卡恢复链内 AGENTS.md
  均未变化（b48984d 与 0be905ab 中 AGENTS.md 一致）。
...
- TASK-0066/0067 恢复记录测试断言改为链内固定历史对象绑定（TASK_0066_BASE_COMMIT 与
  TASK_0067_BASE_COMMIT/TERMINAL_COMMIT 之间 AGENTS.md byte-for-byte 一致），不删测、不加 skip。
...
以 YAML `requiredCommands` 精确 argv 为准。实现迭代只运行 wrapper 与 TASK-0066/0067 相关定向
测试；冻结候选后依次执行独立 Reviewer、唯一 Precheck、完整 Harness、唯一无参数 diff check、
