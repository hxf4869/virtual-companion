# TASK-0111：Auth 输入、错误与身份最小化闭环（P2-04 + P3-05 + P3-06）
...
authorizationCommit: "90e1bc36341c6227966a7c4b96bf1066ab6668ea"
contextFingerprint: deadcf31035ec180393998f0f0abf3ae77a5d3a3d07f49b83033a8242fe9a6a9
contextLock: docs/tasks/context/TASK-0111.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C3
  surfaceId: TASK_0111_AUTH_INPUT_HYGIENE
...
humanApprovals:
  - scope: task-assignment
    ...
    evidence: >-
      ...
      本卡按 TASK-0110 Handoff 的唯一下一动作处理 P2-04/P3-05/P3-06；授权在不触碰数据库
...
> 只追加 `INVALID_REQUEST` 并确定性再生对应 Catalog/OpenAPI 产物。TASK-0110 保持 REJECTED 历史不变。
...
- 不修改 TASK-0102、TASK-0109、TASK-0110 的历史 Task/Evidence/Handoff。

## 输入和前置条件

Base 为 TASK-0110 REJECTED 终态 `a3a294d19b8024bc52fa08eb052fa7a957b046ee`，仓库在 DRAFT 前与
`origin/main` 0/0 且工作树干净，无活动任务。Context Lock 只绑定 Base Commit 中的机器真源、前序 Auth
合同与实现、审计 Handoff、TASK-0110 远端不可用证据、生成器和相关测试。
...
输出 `docs/evidence/TASK-0111/`（候选身份、R1/R2、定向与正式命令、Catalog/OpenAPI diff、远端非 PASS 和
本地 exact-tree 限定覆盖）及 `docs/handoffs/TASK-0111.json`，终态原子更新 Task/Project State/Ledger。
