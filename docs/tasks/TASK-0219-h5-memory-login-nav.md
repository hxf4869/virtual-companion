...
baseCommit: bcf182590310135e67a0faf28fe4f7b94dff7e74
authorizationCommit: cc6192ef7df3a574f8989b97dcd5a227682b6693
contextFingerprint: cd33e9b1aec82879c039669159e771e19ca4e88721fbe26bd5ddb48c7cc44054
contextLock: docs/tasks/context/TASK-0219.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C2
  surfaceId: TASK_0219_H5_MEMORY_LOGIN_NAV
...
writeAllowlist:
  ...
  - docs/tasks/context/TASK-0219.context-lock.yaml
...
humanApprovals:
  - scope: task-intake
    ...
    evidence: >-
      ...
      产品细节与合法 task-intake。本卡只在记忆页增加登录入口，不改变确认/失败语义。
      不编造 sourceThreadId。
...
> 本卡为独立延续单卡。永久 ID 由机器派生：Ledger 最大编号 TASK-0218，intake 分配 TASK-0219。

## 背景与用户可观察目标

聊天页已有登录入口，记忆页没有。用户必须先回边界台再进登录。

## 范围内

- 仅改 `memory.vue` / `memory.spec.ts`。
- 增加 `data-testid=nav-login`「登录」，导航到 `/pages/login/login`。
- 未填 relationship id 时也可见。点击不调用 load/confirm。
...
- Base：`bcf182590310135e67a0faf28fe4f7b94dff7e74`（TASK-0218 ACCEPTED）。
...
1. 记忆页在未填 relationship id 时也可见登录入口。
2. 点击导航到 `/pages/login/login`，不调用 load/confirm。
3. 既有预填/选择器/导航测试继续 PASS。
