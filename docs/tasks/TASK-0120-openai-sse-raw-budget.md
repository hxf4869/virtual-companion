# TASK-0120：OpenAI SSE 连接级原始字节预算
...
authorizationCommit: 7ef5e2a0fd2aa1478918b8acc6d023bebf560b18
contextFingerprint: e44ae53e328a4fb6bb35bf2493ee860e0941fbbbcb390e6d9f451beb3112da9e
contextLock: docs/tasks/context/TASK-0120.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C3
  surfaceId: TASK_0120_OPENAI_SSE_RAW_BUDGET
...
readAllowlist:
  ...
  - docs/evidence/TASK-0119/evidence-pack.json
  - docs/evidence/TASK-0119/review-r1.md
...
writeAllowlist:
  ...
  - service/adapters/model-openai/src/main/java/com/virtualcompanion/modelopenai/OpenAiChatCompletionsSession.java
  - service/adapters/model-openai/src/test/java/com/virtualcompanion/modelopenai/SseDecoderTest.java
  - service/tests/openai-chat-completions-contract-tests/src/test/java/com/virtualcompanion/modelopenai/contract/OpenAiChatCompletionsRawBudgetContractTest.java
forbiddenPaths:
...
humanApprovals:
  - scope: task-assignment
    ...
    evidence: >-
      Owner 明确要求 Codex 不逐项询问并作为长线任务持续推进。TASK-0119 Handoff 与
      project-state 的唯一下一动作逐字指定创建 TASK-0120，为 OpenAI SSE 冻结不可重置的
      8388608-byte 全连接 raw budget；本卡不追溯改写历史任务。
  - scope: openai-sse-stream-raw-byte-budget
    approvedBy: repository-owner
    approvedAt: "2026-08-09"
    sourceThreadId: codex-audit-fix-20260809
    evidence: >-
      Owner 长线授权明确把其余工程数值、拆卡、实现与验证细节交由 Codex 按机器真源、现有模式和
      最小风险原则决定。TASK-0119 的已接受 Handoff 与当前 project-state 已明确授权本卡在 OpenAI
      streaming 全连接冻结 8388608 bytes raw budget；所有实际读取的 data/comment/payload/framing/
      CR/LF/空行与 [DONE] 字节从连接开始累计且永不按 frame、合法 data 或进展信号重置。
...
> 本卡为独立延续单卡，不写 planningBacklog/planningContractHash；永久 ID 已核对未占用。TASK-0119
> 已在当前 Base ACCEPTED 并把 TASK-0120 冻结为唯一下一动作。本卡从其终态提交重新冻结 Context，
> 以独立候选 Commit/Tree、Reviewer 和本地 exact-tree channel 关闭 OpenAI streaming raw 输入放大。

## 背景与用户可观察目标

OpenAI streaming response 当前只有单物理行和单个组合 data event 的 1 MiB 上限；每个换行或 dispatch
都会重置局部计数。Provider 可持续发送合法短 comment、blank 或不产生文本的 data frame，在 total timeout
前持续消耗网络、逐字节解析 CPU、response socket 和虚拟线程；这些字节不受 output 或事件队列上限约束。

完成本卡后，每条 OpenAI SSE 连接从首个实际读取 byte 起拥有不可重置的 8 MiB raw budget。所有字段、
payload、framing、换行与 `[DONE]` 都计数；exact 可完成，首个 one-over byte 在解析/dispatch 前失败为唯一
MalformedResponse，body 关闭且不读取 sentinel。合法 SSE、`[DONE]` early stop、取消、timeout 与既有边界保持。
...
- 为 package-private `SseDecoder.decode` 增加显式 `maximumRawBytes` 参数，必须为正 `long`；在每次成功
  ...
  时抛无详情 `OpenAiCodecException`，不处理该违规 byte，底层最多读取 budget+1。
- raw 预算计入 `data:`、JSON payload、comment、字段 framing、CR、LF、CRLF、空行、blank frame、
  `[DONE]` 和 EOF 前所有真实 bytes；不按物理行、事件 dispatch、有效 data 或任何进展信号重置。
- `OpenAiChatCompletionsSession` 冻结 adapter-private `8 * 1024 * 1024` streaming raw 常量并传给 decoder；
  不修改或复用名为 non-stream 的 `SizeLimits.MAX_NON_STREAM_RESPONSE_BODY_BYTES`。
- 扩展 direct decoder tests，以小预算证明 comment/blank/empty-data/data、CR/LF/CRLF、合法 data 后继续累计、
  unknown `event:`/`id:`/`retry:` 既有 fail-closed，以及 exact/one-over/sentinel/consumer early-stop 行为。
- 新增独立 contract test，以受控 InputStream/HttpClient 覆盖生产 8 MiB exact 成功、one-over early stop、
  comment/blank/contentless-data flood、不 reset、MalformedResponse、body close、`[DONE]` late sentinel、
  structured partial fence、cancel/timeout 和无 late event。

## 明确范围外

- 不修改 TASK-0109 至 TASK-0119 的 Card、Context、Evidence、Review 或 Handoff，不重判历史结果。
- 不修改 BoundedInputStream、SizeLimits/modelruntime、公开协议、Anthropic/fake/failure adapter、OpenAI
...
- 不把空 `data:` frame 改成 heartbeat：decoder 仍可 dispatch 空字符串，session codec 会立即把空 JSON
  判为 malformed。`event:`、`id:`、`retry:` 仍是未知字段并 fail closed，不扩展 SSE 兼容字段。
- 不改变 `[DONE]` 的前置状态、成功 terminal 批次或 early-stop：budget 内 `[DONE]` 完成后 decoder 仍
  立即停止且不读取任何 late byte；overflow 测试必须发生在 `[DONE]` 之前。

## 输入和前置条件

Base 是 TASK-0119 ACCEPTED 终态 `eb35feb03f0718bf3dc193fc675613234f007d72`，与 `origin/main`
0/0 且工作树干净。Context Lock 绑定该 Base 的机器真源、TASK-0113/0116/0119 已接受边界证据、当前
OpenAI raw decoder/session/contract tests、Anthropic 已接受对称实现、modelruntime 限制与精确 Skill。

现有 first-token/total timeout 提供时间边界但不是 byte 边界；TASK-0113 的单行/单 event fence和
TASK-0116 的事件队列背压均不观察被忽略的 raw frames。本卡数值由 Owner 长线工程决策授权独立冻结。
...
- 合法 data、comment、CR/LF/CRLF、EOF 和 `[DONE]` 语义不变；raw budget 不关心 frame 是否产生
  ModelProtocol event、content、usage 或是否被 consumer 忽略。
...
comment/body、prompt、schema、API key、token、真实联系人或用户数据；测试仅用 synthetic bytes。
...
- raw overflow 抛 `OpenAiCodecException`，由 session 归一化为唯一 MalformedResponse；worker finally
  关闭 body，失败路径中止 parser，不产生 Usage/EOS 或成功 terminal。
- budget 内 `[DONE]` 继续要求既有 content、finish reason 与 usage，完成成功终态并令 consumer 返回 false；
  decoder 不读取其后的 late sentinel，raw fence 不延迟或覆盖成功终态。
...
8 MiB 是 OpenAI adapter 私有运维输入上界，不是 SSE/OpenAI 协议能力声明，也不扩大模型权限。

## 验收标准

1. 生产路径只在 OpenAI `SseDecoder` 与 session 增加不可重置的 8388608-byte 全连接 raw fence；
   `SizeLimits`、BoundedInputStream、Anthropic、公开协议与既有 token/output/event/queue/timeout 数值不变。
2. direct decoder 以小预算覆盖纯 comment、blank、empty-data、合法 data；data dispatch 前后均不重置，
   LF、CR 与 CRLF 的每个真实 byte 计入，exact 通过，one-over 只读取 budget+1 且不读 sentinel。
3. production budget 下，8 MiB exact 的合法完整 SSE 含 `[DONE]` 并产生原有 Output/Usage/EOS；one-over
   在违规 byte 处理前产生唯一 MalformedResponse，tracking body 证明读取上界、body close、无违规 Usage/EOS。
4. comment/blank/contentless-data flood 均不能绕过 raw budget；空 data 继续首次 dispatch 后按空 JSON
   malformed，`event:`/`id:`/`retry:` 继续未知字段 fail closed，单行/单 event/strict UTF-8 语义不回归。
5. 合法 delta 后的 raw flood 证明预算不 reset；先前 delta 顺序、binding、sequence 保持，失败 terminal
   唯一且最后，terminal 后 `next()` 永久 empty，late sentinel 未读取；structured partial 不得泄漏。
6. budget 内 `[DONE]` 后的 sentinel 不读取；缺 content/finish reason/usage、EOF 无 `[DONE]` 与 early-stop
   失败语义不变，不把 late byte 误算成已结束连接的 overflow。
7. 持续 raw 输入或阻塞 body 下的 cancel/close、first-token timeout 和已有 content 后 total timeout 有界完成，
   body 关闭、线程停止、terminal first-wins；重复 cancel/close 幂等且无 late event。
8. 既有 normal/structured streaming、non-BMP/multiline/CRLF、boundary、backpressure、failure、timeout 与
   maxTokens 合同全部回归；所有候选路径位于 writeAllowlist，不访问真实 Provider。
9. 定向 reactor、根级 Maven verify、canonical precheck 与唯一无参数 `git diff --check` 均在独立
...
复用 non-stream 命名常量、扩展 SSE grammar、删除测试、增加 skip 或吞退出码。候选冻结后 Reviewer 发现
阻塞项时，按最多一个 fix batch、最多 R2 处理；需修改公共常量/协议、Anthropic 或越界文件则失败关闭。

## 停止条件

- 需要修改 writeAllowlist 外路径、BoundedInputStream、SizeLimits/modelruntime、公开协议、Anthropic、runtime、
  数据库或 CI。
- 需要改变 8 MiB、event/output/queue/token/timeout 数值，按 frame/data 重置预算，扩大 SSE 字段兼容，
  或改变 `[DONE]` 的成功前置条件与 early-stop。
- READY Doctor、Reviewer、canonical、exact-tree 或 pre-closure 任一非 PASS，或触发 90 分钟 hard fuse。

## Evidence Pack

输出 `docs/evidence/TASK-0120/` 与 `docs/handoffs/TASK-0120.json`；记录候选身份、R1/R2、8 MiB exact/
one-over、各 framing/line-ending/data 计数、no-reset、budget+1/sentinel、`[DONE]` early stop、
MalformedResponse、body close、cancel/timeout/late-event、正式门禁、远端非 PASS 与本地 exact-tree 限定覆盖。
终态只用单父原子提交更新任务卡、Project State、Task Ledger、Evidence 与 Handoff，随后 push、fetch 并验证
远端 0/0。Handoff 必须保留其余审计风险，并给出与 `project-state.nextAction` 逐字一致的唯一下一动作。
