# TASK-0124：Auth Firewall Envelope 与 Observation 保真
...
authorizationCommit: 952800e80994c734a7cd40e9631f60110fce85d5
contextFingerprint: 6ac0b7d95a19f7456f20a9496a212deb364a0b1664de1d37a616d41e48d739d2
contextLock: docs/tasks/context/TASK-0124.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C3
  surfaceId: TASK_0124_AUTH_FIREWALL_ENVELOPE_OBSERVATION
...
readAllowlist:
  ...
  - docs/evidence/TASK-0123/review-r1.md
...
humanApprovals:
  - scope: task-assignment
    ...
    evidence: >-
      Owner 要求 Codex 使用当前思考深度、不启用 fast，持续执行既有长线 goal 且不为低风险细节暂停。
      TASK-0123 已真实 REJECTED、推送并远端 0/0；其 Handoff 与 project-state 唯一 nextAction 明确要求
      通过 task-intake 创建 TASK-0124，不重开第三轮修复。
  - scope: auth-firewall-envelope-observation
    approvedBy: repository-owner
    approvedAt: "2026-08-09"
    sourceThreadId: 019fdf92-9c99-7400-a2ea-d490ef3d7e15
    evidence: >-
      TASK-0123 R2 与终态 Handoff 精确授权后续永久任务：补全 encoded Auth-prefix MVC 等价路径的固定
      INVALID_REQUEST envelope，保留 Spring Security 默认 observation marking 和非 Auth 400 行为，并重新
      覆盖 TASK-0123 完整矩阵；只修改 shared helper 与两份聚焦测试，不触碰 Security config 或业务语义。
...
reviewers:
  - id: task0124_r1
    ...
    evidencePath: docs/evidence/TASK-0124/review-r1.md
    reason: 'READY Doctor 真实 FAIL：54 个 Context 输入 hash 全部正确，但 fingerprint payload 错误追加了末尾 LF；canonical fingerprint 应为 855d3fb37679a185be843c3843fb7a7e035f39c87f9ebd435694881265a6bb46。授权字段已冻结、无 IN_PROGRESS 或 candidate，必须 REJECTED 并使用新永久 TASK-0125。'
    candidateTree: bac8ea7a88cb87830c0b12b0d7c4c491355acad0
requiredCommands:
  - python scripts/harness/precheck.py --task TASK-0124
...
> 本卡是 TASK-0123 真实 REJECTED 终态授权的新永久任务，不重开其第三轮 fix/review，不把其未运行的正式
> 门禁转换为 PASS。Backlog 中没有 TASK-0124，因此不写 `planningBacklog` 或 `planningContractHash`。
...
handler 替代了 Spring Security 7.1 默认 observation composition。本卡仅闭合这两个已证实 P2，并重新证明
TASK-0123 的完整 Auth 边界矩阵。
...
- 不修改 TASK-0123 的 Task/Evidence/Handoff，不将其改为 ACCEPTED，不运行或补写其正式门禁。

## 输入和前置条件

- Base 是 TASK-0123 已推送且远端 0/0、post-terminal Doctor PASS 的 REJECTED 终态提交
  `68dc7f0a486e93768112fca856f9456cb7e31a2d`；其候选实现继续存在于历史，但没有 PASS 声明。
- Context Lock 固定 Base 中 54 个输入，包含 R1/R2、Handoff、当前 helper/filter/security wiring/tests 与治理真源。
...
- lowercase/non-POST、malformed 非 Auth、parseable 非 Auth 继续委托默认 status handler；不得被改写成 Auth envelope。
...
- 候选前仅在三条业务白名单路径内前向修复；不得恢复或重写 TASK-0123 历史。
...
- 需要修改 forbidden `AuthSecurityConfig`、依赖、specs、database 或历史 TASK-0123 才能闭合。
...
输出 `docs/evidence/TASK-0124/` 与 `docs/handoffs/TASK-0124.json`，绑定 Base、Context、候选 Commit/Tree、Reviewer、
