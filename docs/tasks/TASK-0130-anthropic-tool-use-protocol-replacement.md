# TASK-0130：Anthropic Tool-Use 协议一致性 Replacement
...
authorizationCommit: eb6236e26f574ff75c3ea39edfab7920d96c67c3
contextFingerprint: 9fc63fcc784e1dfeb554c5659ce5107344a8a98625420a782b427529d1bff718
contextLock: docs/tasks/context/TASK-0130.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C3
  surfaceId: TASK_0130_ANTHROPIC_TOOL_USE_PROTOCOL_REPLACEMENT
...
humanApprovals:
  - scope: task-assignment
    ...
    evidence: >-
      ...
      TASK-0129 已因冻结 Context fingerprint 错误而 REJECTED、推送并远端 0/0，其 Handoff 与
      project-state 唯一 nextAction 指向本 replacement。
  - scope: anthropic-tool-use-protocol-replacement
    approvedBy: repository-owner
    approvedAt: "2026-08-10"
    sourceThreadId: 019fdf92-9c99-7400-a2ea-d490ef3d7e15
    evidence: >-
      Owner 长线授权要求失败任务保留事实并以前向永久 replacement 继续。TASK-0129 未进入
      IN_PROGRESS、没有业务候选；其 Base 仍保留 TASK-0128 的实现候选与 R1 P3 修复。本卡只在正确的
      canonical Context fingerprint、全新授权与预算下重新审查和验证同一协议修复，禁止继承旧 PASS。
...
> 本卡不在 Backlog 中，因此不写 `planningBacklog` 或 `planningContractHash`。本卡是 TASK-0129 的永久
> replacement；TASK-0129 只发生 intake fingerprint 失败。本卡只能前向采用 Base 已包含的实现并重新取得
> 独立 Review/validation 证据，不改写旧任务。

## 背景与用户可观察目标

TASK-0128 已实现 Anthropic structured tool name 与 stream block index 一致性，并有完整 contract tests，
但因 R2 超过 Reviewer 15 分钟硬上限而正确 REJECTED。TASK-0129 试图重新授权同一实现，却因 Context
fingerprint payload 错误追加末尾 LF 而在 READY Doctor 失败，并在没有候选的情况下正确 REJECTED。
用户可观察目标保持不变：合法 Anthropic text/structured 回包继续成功；错误 tool name、跨 block delta/stop、
第二 structured block 均 fail closed，且该行为这次由期限内独立 Reviewer 和同一 clean candidate 的完整
正式门禁证明。
...
- 生成全新的 TASK-0130 Evidence/Handoff、terminal commit、push/fetch 与远端 0/0 证据。

## 明确范围外

- 不继承 TASK-0128 Reviewer、formal command 或 local exact-tree 的 PASS；TASK-0129 无候选 PASS；不改两卡历史产物。
...
- Base `ebc7c2228c5ed4d07ec345d9d33a7d187de00392` 是 TASK-0129 REJECTED 单父终态，已 push、fetch、
  `HEAD...origin/main=0/0` 且 post-terminal Doctor PASS。
- Base 保留 TASK-0128 的实现与测试提交；TASK-0129 没有业务候选或可复用 PASS。不得撤销实现、复制
  PASS、重用时间锚或 Reviewer output。
- Context Lock 固定 72 个治理、历史、协议、实现与测试输入。
...
4. TASK-0128、TASK-0129 的 Card/Context/Evidence/Reviews/Handoff 与所有历史任务零 diff；provider 配置、limits、OpenAI、DB/specs 零 diff。
...
7. Evidence/Handoff 如实区分本卡新证据、TASK-0128 的 late/non-promotable 结果与 TASK-0129 的 intake FAIL；remote 非 PASS 不冒充 local PASS。
...
- 不回滚或改写 TASK-0128、TASK-0129；当前 Base 是唯一前向起点。
...
输出 `docs/evidence/TASK-0130/` 与 `docs/handoffs/TASK-0130.json`，绑定新的候选 Commit/Tree、Reviewer、
