# TASK-0116：OpenAI 流式事件有界队列与可取消背压
...
authorizationCommit: "c03b61a058aad6d06e0b2ade87428ca0b5128818"
contextFingerprint: 86ec3c720b3c89be598764340a64afdf6ff961f5fa189a7bd32b1261758b1bb4
contextLock: docs/tasks/context/TASK-0116.context-lock.yaml
...
complexityAssessment:
  ...
  riskClass: C3
  surfaceId: TASK_0116_OPENAI_STREAM_BACKPRESSURE
...
humanApprovals:
  - scope: task-assignment
    ...
    evidence: >-
      ...
      TASK-0115 Handoff 与 project-state 的唯一下一动作逐字指定创建 TASK-0116，为 OpenAI streaming
      session 引入有界事件队列与可取消 backpressure；本卡不追溯改写历史任务。
  - scope: openai-stream-backpressure-capacity
    approvedBy: repository-owner
    approvedAt: "2026-08-09"
    sourceThreadId: codex-audit-fix-20260809
    evidence: >-
      Owner 长线授权明确将其余工程数值、拆卡、实现与验证细节交由 Codex 按机器真源、现有模式和
      最小风险原则决定。本卡冻结最多 64 个待消费 OutputDelta，并为 structured/non-stream 成功批次
      保留 3 个控制事件位置，因此任何时刻最多保留 67 个事件引用；该值不成为公开配置或协议字段。
  - scope: local-exact-tree-fallback
    ...
    evidence: >-
      ...
      在无 runner、零 step 状态终止；TASK-0112 至 TASK-0115 已按同一 READY 冻结 fallback 合规
...
reviewers:
  - id: task0116_r1
    ...
    evidencePath: docs/evidence/TASK-0116/review-r1.md
    reason: 'R1 完整矩阵复核 PASS：候选 Commit/Tree、Context、逐父授权与 writeAllowlist 一致；64 个 pending OutputDelta、3 个控制事件保留位、wait/notify、取消/超时/body-close、连续序列和唯一终态均满足验收；P0/P1/P2/P3 为零。'
    candidateTree: 82ffa19d59bb5db003059671ae14d83f38b09984
requiredCommands:
  - python scripts/harness/precheck.py --task TASK-0116
...
> 本卡为独立延续单卡，不写 planningBacklog/planningContractHash；永久 ID 已核对未占用。TASK-0115
> 已在当前 Base ACCEPTED 并把 TASK-0116 冻结为唯一下一动作。本卡从其终态提交重新冻结 Context，
> 以独立候选 Commit/Tree、Reviewer 和本地 exact-tree channel 关闭 OpenAI 无界事件队列。

## 背景与用户可观察目标

`OpenAiChatCompletionsSession` 当前使用无容量参数的 `LinkedBlockingQueue`。Provider 可以持续发送
空或极小的合法 delta，而调用方消费更慢或暂时不调用 `next()`；即使单事件与累计输出字节均受限，
解析线程仍会无限创建并排队事件对象，最终耗尽进程内存。

完成本卡后，OpenAI session 最多缓冲 64 个待消费 `OutputDelta`，并固定保留最多 3 个成功控制事件
位置，解析线程在容量边界阻塞且释放状态锁；消费、cancel、timeout 与 close 都能唤醒它，body 最终关闭，
唯一终态不会因队列满而丢失或死锁。

## 范围内

- 仅在 package-private `OpenAiChatCompletionsSession` 内以 `ArrayDeque` 和同一 `stateLock` 实现有界
  producer/consumer：最多 64 个待消费 OutputDelta；成功批次最多再加入 structured/non-stream final
...
- cancel/close/total-timeout 在满容量时也必须有界返回，关闭 response body、取消 future、interrupt parser
  与 worker；已接受事件保持顺序，terminal 最后且唯一，terminal 后 `next()` 永久 empty。
- 新增独立 OpenAI contract test 文件，以自定义受控 HttpClient/InputStream 与 loopback 路径覆盖容量、
  一进一出恢复、满载 cancel、满载 timeout、body close、唯一终态、序列连续与 success/failure 回归。

## 明确范围外

- 不修改 TASK-0109 至 TASK-0115 的 Card、Context、Evidence、Review 或 Handoff，也不重判历史结果。
- 不修改 ModelProtocolSession/Event、SizeLimits、Utf8ByteAccumulator、公开 API/OpenAPI/Catalog、OpenAI
  config/codec/adapter/SSE parser、Provider 路由、授权、quota/retry、数据库、前端或 CI。
- 不修改累计 output、单 SSE event、raw response body、timeout 或 token 数值；64/3 仅是 session 私有
  内存背压上界，不进入公开请求/响应或 runtime 配置。
- 不解决 Anthropic 事件 queue/backpressure、Anthropic maxTokens ceiling，或连续 comment/empty-data
  frame 的连接级累计 raw budget；这些风险继续拆分为后续独立卡。
- 不保证调用方永不停止消费时 session 可以成功完成；它必须保持内存有界，并由既有 total timeout 或
  显式 cancel 产生唯一失败/取消终态。

## 输入和前置条件

Base 是 TASK-0115 ACCEPTED 终态 `1ad813579d235b45ab511f75be1aa6fe1bf3bb14`，与 `origin/main`
0/0 且工作树干净。Context Lock 只绑定该 Base 的机器真源、TASK-0113 至 TASK-0115 已接受边界证据、
模型协议合同、当前 OpenAI/session/Invoker 调用链与相关 contract tests，不依赖历史聊天。
...
- 64 是“尚未被 `next()` 移除的 streaming text OutputDelta”上限；parser 可持有当前正在处理但尚未入队
  的一个 chunk。structured streaming 在 parser 内聚合，完成时原子加入 final delta、Usage 与 EOS。
- consumer 移除事件即释放一个缓冲位置；不要求下游处理完成确认。Provider socket 的 OS 缓冲不属于
...
真实联系人或用户数据；全部测试使用 synthetic bytes、受控内存 HttpClient 或 loopback server，不触发真实网络。
...
- success：under lock 原子加入最多 final delta、Usage、EOS，再冻结 terminal 并通知全部等待者；保留的
  3 个位置保证不因容量边界失败。
- failure/cancel：under lock 冻结唯一 terminal、加入单个终态并通知全部等待者；随后 lock 外 abort I/O，
  满队列时也不得等待 consumer 才能返回。
- terminal 冻结后 parser 不得再入队或继续消费 provider bytes；body close 和 thread interrupt 必须幂等。
...
1. 生产实现不再创建无界 `LinkedBlockingQueue`；在无 consumer 的 streaming text flood 下最多缓冲 64 个
...
4. 队列已满且 consumer 停止时，既有 total timeout 可在预算内冻结唯一 `AttemptFailed(Timeout(TOTAL))`、
   关闭 body并唤醒 parser；恢复 drain 不死锁，无 Usage/EOS 或 late delta。
5. `next()` 在空队列等待时被 interrupt，必须保留线程 interrupt flag、触发唯一取消终态且不遗留 worker/
   parser 等待；既有 connect、first-token、total-timeout contract 全部保持通过。
6. structured streaming 与 non-stream success 能原子容纳 final delta、Usage、EOS 三事件；failure/cancel 终态
   不依赖空闲 output slot，所有路径 body 最终关闭。
7. 生产代码只修改 OpenAI session；测试只新增独立 backpressure contract 文件。Anthropic、modelruntime、
...
- 需要修改 writeAllowlist 外路径、公开协议、SizeLimits、OpenAI config/codec/adapter/SSE parser、
  Anthropic、runtime app、数据库或 CI。
...
输出 `docs/evidence/TASK-0116/` 与 `docs/handoffs/TASK-0116.json`；记录候选身份、R1/R2、容量与
producer/consumer race matrix、cancel/timeout/body-close、正式门禁、远端非 PASS 与本地 exact-tree
限定覆盖。终态只用单父原子提交更新任务卡、Project State、Task Ledger、Evidence 与 Handoff，
随后 push、fetch 并验证 `HEAD...origin/main` 为 0/0。Handoff 必须保留 Anthropic queue/backpressure、
Anthropic maxTokens ceiling 与跨 frame raw budget 风险，并给出与 `project-state.nextAction` 逐字一致的唯一下一动作。
