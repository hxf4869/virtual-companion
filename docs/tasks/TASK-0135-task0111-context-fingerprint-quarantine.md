...
closureOnly: true
terminalStateReason: >-
  READY 后 Owner 批准将隔离范围扩大为 TASK-0111/0124/0129 三个固定 fingerprint 历史，但 READY 冻结
  的任务卡授权投影不可修改（Doctor 强制 title/body/immutable metadata 在 READY 后不变）。候选提交把三历史
  身份与范围说明写入任务卡正文，导致唯一正式 Precheck 在 Doctor 阶段如实 FAIL（authorization projection
  changed）。隔离实现本身经独立 R1 PASS（P0/P1/P2=0），但正式门禁非 PASS 即停止 promotion，本卡按机器
  策略如实 REJECTED，不改写历史、不删测、不伪造 PASS；三历史隔离由新的 C4 治理卡从 DRAFT 起冻结后
  重新验证并正式承接 P2-20。
...
reviewers:
  - id: task0135_r1
    ...
    evidencePath: docs/evidence/TASK-0135/review-r1.md
    reason: "R1 完整复核 PASS：三历史固定身份（commit/tree/parent/diff 路径集）、验收标准逐条、INV-HARNESS-001..009 与邻近风险全部核对；隔离 predicate 只对 TASK-0111/0124/0129 三个固定 REJECTED 身份放行，普通 context-lock 校验路径零改动，无 CLI/env/alias 兼容层；P0/P1/P2=0、P3=3 非阻断提示；Reviewer 未运行正式门禁。"
