...
> 版本：2026-08-22
> 初始审计快照：`485a87fa`；最近一次对账基线：`90e85358`
...
| 候选 | 当前判断 | 下一步边界 |
|---|---|---|
| `A4 Owner-only dogfood` | `IN_PROGRESS`（Owner 决策见 ADR-0006） | 连续 7 天探索性自体验，不设通过阈值；不能代替 D0、`GO/PIVOT/STOP` 或真实用户 Beta 门禁 |
...
## 14. Owner 决策与仍未满足的外部条件

2026-08-24，Owner 已接受
[`ADR-0006`](../decisions/0006-owner-only-local-dogfood-boundary.md)，冻结唯一内部账号的
本机 7 天 dogfood 边界：单实例、同源、全天可用、WeChat ChatAPI generation、同模型
输入/最终安全分类、本地确定性 embedding + Qwen 影子评测、本机 MinIO、30 天对话保留、
7 天加密备份、iPhone Safari + Android Chrome 和简体中文无障碍冒烟。完整规则只在
ADR-0006 维护，本路线图不复制为第二套真源。

这些选择只解决 Owner-only dogfood 的执行输入。以下事项仍不得由 Agent 猜测或代填：

1. 当前无可招募样本，D0 延期；仍没有合格的访谈/原型体验样本，也没有
   `GO/PIVOT/STOP` 决策。
2. WeChat ChatAPI 的权威条款、处理地区、供应商保留期、训练使用和删除机制仍待 Owner
   提供登录后证据并复核；此前敏感内容、账号元数据和记忆片段外发保持关闭。
3. 真实用户 Beta 的 PIA/DPA、真实成年核验供应商、紧急联系人渠道、法务/专业评审、
   值班替补与升级链仍未闭环。
4. 真实 Beta 的远端环境、域名、备案、运维账号、生产 KMS/密钥轮换、外部删除 manifest
   存储和正式 retention policy 仍未批准。
5. 管理员 MFA、多人 case 角色和实际人类模块 Owner 仍未确定；没有真实协作者前不得创建
   `CODEOWNERS` 或声称完成生产发布审批。
6. 真实供应商账单与费用容差仍未知；套餐截图只可作为本机临时配置输入，不能进入机器
   真源，也不能把未知状态显示为 0 成本。

Agent 遇到上述外部条件时，应把相关交付标记为 `BLOCKED_EXTERNAL`，随后继续执行与其
独立的本地任务；不得绕过门禁、虚构批准或把 Owner dogfood 当作正式 Canary/Beta。

## 15. 文档维护边界

- 本文初始审计基于 `485a87fa`，最近对账到 `90e85358`，不是新的机器真源；实现后以 Catalog、OpenAPI、代码、migration 和 README 为准。
