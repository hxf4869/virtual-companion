...
| 9 | PostgreSQL 18 + pgvector V1-V15 迁移（38 行） | `service/platform/persistence/src/main/resources/db/migration/` 15 个 V1..V15 文件；`infra/db/run-rls-tests.sh` 的 pgvector/pgvector:0.8.5-pg18 镜像 | 一致 |
...
| 16 | MANIFEST.sha256 仅证明 V0.3.1 起步包来源（95 行） | `git log --oneline -- MANIFEST.sha256` 唯一 commit `85cc739 "chore: 初始化V0.3.1技术契约基线"`，自初始化后从未更新，确不代表当前仓库完整性 | 一致 |
