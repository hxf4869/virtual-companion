#!/usr/bin/env node
// G12 fake OpenAI-compatible provider. Never logs request bodies, headers, or tokens.
...
const config = {
    firstTokenMs: Number(process.env.G12_FIRST_TOKEN_MS ?? "200"),
    deltaMs: Number(process.env.G12_DELTA_MS ?? "50"),
    chunks: Number(process.env.G12_CHUNKS ?? "8"),
    holdMs: Number(process.env.G12_HOLD_MS ?? "0"),
};
...
async function handle(request, response) {
    ...
    if (request.method === "GET" && url.pathname === "/g12/config") {
        sendJson(response, 200, { ...config });
        return;
    }
    if (request.method === "GET" && url.pathname === "/g12/stats") {
        sendJson(response, 200, { ...stats, config: { ...config } });
        return;
    }
    if (request.method === "PUT" && url.pathname === "/g12/config") {
...
server.listen(PORT, HOST, () => {
    process.stdout.write(`G12_FAKE_PROVIDER_READY host=${HOST} port=${PORT}\n`);
});
