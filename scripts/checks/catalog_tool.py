...
def validate(root: Path) -> list[str]:
    ...
    if tech.get("currentRuntime") != "GO_COMPANIOND":
        errors.append("product-scope.yaml: technology.currentRuntime must be GO_COMPANIOND")
...
def validate_go_v1_api_scope(root: Path) -> list[str]:
    ...
    if not isinstance(undeclared, list) or not undeclared:
        ...
        for item in undeclared:
            if not isinstance(item, dict):
                errors.append("go-v1-api-scope.yaml: undeclaredLegacyOperations entries must be objects")
                continue
...
def generate(root: Path, out: Path) -> None:
    ...
    for d in (ts_dir, openapi_dir, sql_dir): d.mkdir(parents=True, exist_ok=True)
    ...
    sql = ["-- GENERATED by catalog_tool.py; DO NOT EDIT.", "-- Reference data only; include through a reviewed schema migration.", "CREATE TABLE IF NOT EXISTS ref_catalog_value (", "  catalog_type varchar(128) NOT NULL,", "  code varchar(128) NOT NULL,", "  ordinal integer NOT NULL,", "  PRIMARY KEY (catalog_type, code)", ");", ""]
    for name, values in enums:
