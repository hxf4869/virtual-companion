...
def generate(root: Path, out: Path) -> None:
    ...
    for name, values in enums:
        ...
        body = (header + "package com.virtualcompanion.catalog;\n\n"
                ...
                + "}\n")
        (java_dir / f"{name}.java").write_text(body, encoding="utf-8")
    ...
    (openapi_dir / "catalog-schemas.yaml").write_text(yaml.safe_dump(openapi, allow_unicode=True, sort_keys=False, width=120), encoding="utf-8")
    ...
    (sql_dir / "catalog-values.sql").write_text("\n".join(sql)+"\n", encoding="utf-8")
    cdir = root / "specs/catalog"
    source = {}
    ...
    (out / "catalog.snapshot.json").write_text(json.dumps(snapshot, ensure_ascii=False, sort_keys=True, indent=2)+"\n", encoding="utf-8")
