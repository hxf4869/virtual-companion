...
class CiExecutionPolicyTests(unittest.TestCase):
    def test_policy_is_registered_and_fail_closed(self) -> None:
        audit = Audit()
        validate_ci_execution_policy(audit)
        self.assertEqual([], audit.errors)

    def test_policy_rejects_unknown_quota_downgrade_and_false_platform_pass(
        self,
    ) -> None:
        ...
        unknown_quota = copy.deepcopy(load_yaml(policy_path))
        unknown_quota["channels"]["PRIMARY_REMOTE_EXACT_SHA"][
            "unavailableEvidence"
        ]["unknownQuotaBehavior"] = "SKIP"
        variants.append(unknown_quota)

        result_downgrade = copy.deepcopy(load_yaml(policy_path))
        result_downgrade["rules"]["resultBasedProfileDowngradeForbidden"] = False
        variants.append(result_downgrade)

        false_macos_pass = copy.deepcopy(load_yaml(policy_path))
        false_macos_pass["profiles"]["HARNESS_PORTABILITY_LOCAL"]["platforms"][
            "macos"
        ]["unavailableStatus"] = "PASS"
        variants.append(false_macos_pass)

        generalized_bootstrap = copy.deepcopy(load_yaml(policy_path))
        generalized_bootstrap["task0064Bootstrap"]["oneTimeOnly"] = False
        variants.append(generalized_bootstrap)

        generalized_recovery = copy.deepcopy(load_yaml(policy_path))
        generalized_recovery["task0066Recovery"]["reusableByOtherTask"] = True
        variants.append(generalized_recovery)

        wrong_recovery_base = copy.deepcopy(load_yaml(policy_path))
        wrong_recovery_base["task0066Recovery"]["baseCommit"] = "f" * 40
        variants.append(wrong_recovery_base)

        wrong_recovery_authority = copy.deepcopy(load_yaml(policy_path))
        ...
        generalized_task0068 = copy.deepcopy(load_yaml(policy_path))
        generalized_task0068["task0068Recovery"]["reusableByOtherTask"] = True
        variants.append(generalized_task0068)

        generalized_task0069 = copy.deepcopy(load_yaml(policy_path))
        generalized_task0069["task0069Recovery"]["reusableByOtherTask"] = True
        variants.append(generalized_task0069)

        false_task0069_remote_pass = copy.deepcopy(load_yaml(policy_path))
        false_task0069_remote_pass["task0069Recovery"]["requiredOutcomes"][
            "githubActions"
        ] = "PASS"
        variants.append(false_task0069_remote_pass)
    ...
    def test_task0069_remote_unknown_local_recovery_is_exact_and_fail_closed(
        ...
        recovery = policy["task0069Recovery"]
        self.assertEqual(
            {
            ...
            recovery["remoteAvailabilityEvidence"],
        )
        self.assertEqual(
            {
            ...
            recovery["localFallbackActivation"],
        )
        self.assertEqual(
            "REMOTE_VALIDATION_REQUIRED",
            policy["channels"]["PRIMARY_REMOTE_EXACT_SHA"][
                "unavailableEvidence"
            ]["unknownQuotaBehavior"],
        )
        real_load_yaml = doctor.load_yaml
        variants = []
        for path, value in (
            (("remoteAvailabilityEvidence", "status"), "PASS"),
            (("remoteAvailabilityEvidence", "dispatchCount"), 1),
            (("remoteAvailabilityEvidence", "currentQuotaVerified"), True),
            (("localFallbackActivation", "scope"), "ALL_TASKS"),
            (("localFallbackActivation", "globalUnknownQuotaBehaviorUnchanged"), False),
        ):
            variant = copy.deepcopy(policy)
            variant["task0069Recovery"][path[0]][path[1]] = value
        ...
        for variant in variants:
            ...
            audit = Audit()
            with patch.object(doctor, "load_yaml", side_effect=load_with_variant):
                validate_ci_execution_policy(audit)
            self.assertTrue(audit.errors)

    def test_task0066_evidence_requires_exact_candidate_and_bounded_coverage(
        self,
    ) -> None:
        ...
        evidence = {
            ...
            "validationChannels": {
                ...
                "results": [
                    result("windows", windows_argv),
                    result("wslUbuntu", wsl_argv),
                ],
                "notCovered": [],
                "deferred": [
                    {
                        ...
                        "residualRisk": "No controlled macOS host is available.",
                        "followUpCondition": "Rerun on an exact-tree macOS host.",
                    }
                ],
                "remote": {
                ...
                "terminalMetadataOnly": {
                    "profile": "TERMINAL_METADATA_ONLY",
                    "implementationCandidateCommit": commit,
        ...
        audit = Audit()
        validate_task0064_local_fallback_evidence(audit, task, evidence)
        self.assertEqual([], audit.errors)

        variants = []
        false_deferred = copy.deepcopy(evidence)
        false_deferred["validationChannels"]["deferred"][0]["status"] = "PASS"
        variants.append(false_deferred)
        wrong_tree = copy.deepcopy(evidence)
        wrong_tree["validationChannels"]["candidateTree"] = "d" * 40
        variants.append(wrong_tree)
        reused_commit = copy.deepcopy(evidence)
        reused_commit["validationChannels"]["results"][0][
            "candidateCommit"
        ] = "e" * 40
...
class DeliveryPolicyTests(unittest.TestCase):
    ...
    def test_durable_command_canonical_byte_domain_is_exact_and_fail_closed(
        ...
        with patch.object(
            doctor,
            "DURABLE_COMMAND_CANONICAL_PATH",
            "scripts/harness/not-the-durable-helper.ps1",
        ):
            doctor.validate_current_durable_command_byte_domain(wrong_path)
        self.assertTrue(wrong_path.errors)
...
class BacklogTests(unittest.TestCase):
    ...
    def test_task0012_owner_amendment_real_history_edge_is_exact_and_atomic(
        self,
    ) -> None:
        parent = doctor.yaml_at_commit(
            doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_PARENT_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        child = doctor.yaml_at_commit(
            doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        self.assertNotIn("authorizationAmendments", parent)
        audit = Audit()
        validate_backlog_history_edge(
            audit,
            parent,
            child,
            (
                f"{doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_PARENT_COMMIT}.."
                f"{doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_COMMIT}"
            ),
            parent_commit=doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_PARENT_COMMIT,
            child_commit=doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_COMMIT,
        )
        self.assertEqual([], audit.errors)

        strict_empty_parent = copy.deepcopy(parent)
        strict_empty_parent["authorizationAmendments"] = {}
        self.assertTrue(
            doctor.authorization_amendment_authority_bootstrap_projection(
                strict_empty_parent,
                child,
                parent_commit=doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_PARENT_COMMIT,
                child_commit=doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_COMMIT,
        ...
        for invalid in (None, [], {"unexpected": {}}):
            ...
            variant["authorizationAmendments"] = invalid
            parent_variants.append(variant)
        for variant in parent_variants:
            with self.subTest(parent_variant=variant):
                self.assertFalse(
                    doctor.authorization_amendment_authority_bootstrap_projection(
                        variant,
                        child,
        ...
        child_variants = []
        wrong_owns = copy.deepcopy(child)
        wrong_owns["authority"]["owns"][-1] = "wrong"
        child_variants.append(wrong_owns)
        extra_authority = copy.deepcopy(child)
        extra_authority["authority"]["extra"] = True
        child_variants.append(extra_authority)
        extra_amendment = copy.deepcopy(child)
        extra_amendment["authorizationAmendments"]["extra"] = {}
        child_variants.append(extra_amendment)
        for variant in child_variants:
            with self.subTest(child_variant=variant):
                self.assertFalse(
                    doctor.authorization_amendment_authority_bootstrap_projection(
                        parent,
                        variant,
        ...
        validate_backlog_history_edge(
            ...
            corrupted,
            "child..corrupt",
            parent_commit=doctor.AUTHORIZATION_AMENDMENT_BOOTSTRAP_COMMIT,
            child_commit="c" * 40,
        )
        self.assertTrue(
    ...
    def test_task0062_rejected_authorization_projection_isolation_is_exact_and_fail_closed(
        ...
        task = discover_tasks()["TASK-0062"]
        authorized_text = doctor.git_object(
            doctor.TASK_0062_AUTHORIZATION_COMMIT,
            doctor.TASK_0062_CARD_PATH,
        ).decode("utf-8")
        current_text = (
            ROOT / doctor.TASK_0062_CARD_PATH
        ).read_text(encoding="utf-8")
        self.assertTrue(
            doctor.task0062_rejected_authorization_history_isolated(
                "TASK-0062",
                doctor.TASK_0062_CARD_PATH,
                task,
                authorized_text,
        ...
        for field, value in (
            ("taskId", "TASK-0063"),
            ("state", "ACCEPTED"),
            ("authorizationCommit", "f" * 40),
            ("baseCommit", "e" * 40),
        ):
            variant = copy.deepcopy(task)
            variant[field] = value
            metadata_variants.append(variant)
        ...
        self.assertFalse(
            doctor.task0062_rejected_authorization_history_isolated(
                ...
                doctor.TASK_0062_CARD_PATH,
                task,
                authorized_text.replace("35 分钟", "34 分钟", 1),
                current_text,
            )
        )
        self.assertFalse(
            doctor.task0062_rejected_authorization_history_isolated(
                ...
                task,
                authorized_text,
                current_text.replace("45 分钟", "46 分钟", 1),
            )
        )
        ...
        ledger_path = ROOT / doctor.TASK_LEDGER_PATH
        drifted_ledger = copy.deepcopy(load_yaml(ledger_path))
        drifted_ledger["tasks"]["TASK-0062"]["state"] = "ACCEPTED"
        ...
        target = ROOT / "docs/evidence/TASK-0062/review-r1.md"
        real_read_bytes = doctor.read_repository_bytes

        def read_with_artifact_drift(path: Path) -> bytes:
            content = real_read_bytes(path)
            return content + b"\ncorrupt" if Path(path) == target else content
    ...
    def test_task0063_terminal_missing_reviewer_isolation_is_exact_and_fail_closed(
        self,
    ) -> None:
        task = discover_tasks()["TASK-0063"]
        self.assertTrue(doctor.task0063_terminal_missing_reviewer_isolated(task))
        ...
        for field, value in (
            ...
            with self.subTest(field=field):
                self.assertFalse(
                    doctor.task0063_terminal_missing_reviewer_isolated(variant)
                )

        wrong_authority = copy.deepcopy(task)
        wrong_authority["humanApprovals"][0]["approvedBy"] = "unauthorized-owner"
        real_read_text = doctor.read_repository_text

        def read_with_wrong_authority(path: Path) -> str:
            content = real_read_text(path)
            if Path(path) == ROOT / doctor.TASK_0063_CARD_PATH:
                return content.replace(
                    "approvedBy: repository-owner",
                    "approvedBy: unauthorized-owner",
                    1,
                )
            return content
        ...
        with patch.object(
            doctor,
            "read_repository_text",
            side_effect=read_with_wrong_authority,
        ):
            self.assertFalse(
                doctor.task0063_terminal_missing_reviewer_isolated(
                    wrong_authority
                )
            )

        with patch.object(doctor, "TASK_0063_TERMINAL_TREE", "0" * 40):
            self.assertFalse(
                doctor.task0063_terminal_missing_reviewer_isolated(task)
            )

        target = ROOT / "docs/evidence/TASK-0063/evidence-pack.json"
        real_read_bytes = doctor.read_repository_bytes
    ...
    def test_task0062_replacement_is_exact_and_atomic(self) -> None:
        current, tasks, _, _ = self.load_inputs()
        child = copy.deepcopy(current)
        child["tasks"]["TASK-0055"]["dependencies"] = ["TASK-0062"]
        parent = copy.deepcopy(child)
        ...
        for task_id in ("TASK-0057", "TASK-0058", "TASK-0059"):
            self.assertEqual(
                canonical_json_sha256(current["tasks"][task_id]),
                tasks[task_id]["planningContractHash"],
            )

    def test_task0064_replacement_authority_is_exact_and_fail_closed(self) -> None:
        parent = doctor.yaml_at_commit(
            doctor.TASK_0064_PLANNING_REPAIR_PARENT_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        child = doctor.yaml_at_commit(
            doctor.TASK_0064_PLANNING_REPAIR_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        self.assertTrue(doctor.task0064_planning_repair_projection(parent, child))
        self.assertTrue(
            doctor.task0064_planning_repair_authorized(
                doctor.TASK_0064_PLANNING_REPAIR_PARENT_COMMIT,
                doctor.TASK_0064_PLANNING_REPAIR_COMMIT,
            )
        )
        ...
        variants = []
        wrong_dependency = copy.deepcopy(child)
        wrong_dependency["tasks"]["TASK-0055"]["dependencies"] = ["TASK-0063"]
        variants.append(wrong_dependency)
        wrong_title = copy.deepcopy(child)
        wrong_title["tasks"]["TASK-0055"]["title"] += " extra"
        variants.append(wrong_title)
        ...
        for variant in variants:
            with self.subTest(variant=variant):
                self.assertFalse(
                    doctor.task0064_planning_repair_projection(parent, variant)
                )
        ...
        real_git_object = doctor.git_object

        def git_object_with_wrong_authority(commit: str, path: str) -> bytes:
            content = real_git_object(commit, path)
            if (
                commit == doctor.TASK_0064_PLANNING_REPAIR_COMMIT
                and path == doctor.TASK_0064_CARD_PATH
            ):
                return content.replace(
                    b"scope: task-0064-local-fallback-bootstrap",
                    b"scope: wrong-task-authority",
                    1,
                )
            return content
        ...
        with patch.object(
            doctor,
            "git_object",
            side_effect=git_object_with_wrong_authority,
        ):
            self.assertFalse(
                doctor.task0064_planning_repair_authorized(
                    doctor.TASK_0064_PLANNING_REPAIR_PARENT_COMMIT,
    ...
    def test_task0066_replacement_is_exact_and_atomic(self) -> None:
        ...
        wrong_dependency = copy.deepcopy(child)
        wrong_dependency["tasks"]["TASK-0055"]["dependencies"] = ["TASK-0064"]
        self.assertFalse(
            doctor.task0066_planning_repair_projection(parent, wrong_dependency)
        )
        expanded = copy.deepcopy(child)
        expanded["tasks"]["TASK-0056"]["objective"] = "scope expanded"
        self.assertFalse(doctor.task0066_planning_repair_projection(parent, expanded))

        task0066 = discover_tasks()["TASK-0066"]
        self.assertTrue(doctor.task0066_repair_approvals_are_exact(task0066))
        wrong_scope = copy.deepcopy(task0066)
        wrong_scope["humanApprovals"][1]["scope"] = "wrong-authority"
        self.assertFalse(doctor.task0066_repair_approvals_are_exact(wrong_scope))
        wrong_base = copy.deepcopy(task0066)
        wrong_base["humanApprovals"][1]["evidence"] = wrong_base[
            "humanApprovals"
        ][1]["evidence"].replace(doctor.TASK_0066_BASE_COMMIT, "f" * 40)
        self.assertFalse(doctor.task0066_repair_approvals_are_exact(wrong_base))
        extra_approval = copy.deepcopy(task0066)
        ...
        self.assertFalse(
            doctor.task0066_repair_approvals_are_exact(extra_approval)
        )
        self.assertFalse(
    ...
    def test_task0067_replacement_is_exact_and_atomic(self) -> None:
        child = doctor.yaml_at_commit(
            doctor.TASK_0067_TERMINAL_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        tasks = discover_tasks()
        parent = copy.deepcopy(child)
        ...
        historical_task0055 = doctor.task_metadata_from_text(
            task0055_text,
            "TASK-0067 historical TASK-0055",
        )
        self.assertEqual(
            canonical_json_sha256(child["tasks"]["TASK-0055"]),
        ...
        for path in (
            ...
            self.assertEqual(
                doctor.git_object(doctor.TASK_0067_BASE_COMMIT, path),
                doctor.git_object(doctor.TASK_0067_TERMINAL_COMMIT, path),
                path,
            )
        for path in (
            "CLAUDE.md",
    ...
    def test_task0067_terminal_missing_reviewer_isolation_is_exact_and_fail_closed(
        self,
    ) -> None:
        task = discover_tasks()["TASK-0067"]
        self.assertTrue(doctor.task0067_terminal_missing_reviewer_isolated(task))
        wrong_task = copy.deepcopy(task)
        wrong_task["reviewers"] = [{"id": "invented"}]
        self.assertFalse(
            doctor.task0067_terminal_missing_reviewer_isolated(wrong_task)
        )
        with patch.object(doctor, "TASK_0067_TERMINAL_TREE", "0" * 40):
            self.assertFalse(
                doctor.task0067_terminal_missing_reviewer_isolated(task)
            )
        with patch.dict(
            doctor.TASK_0067_TERMINAL_ARTIFACT_SHA256,
            {doctor.TASK_0067_CARD_PATH: "0" * 64},
        ):
            self.assertFalse(
                doctor.task0067_terminal_missing_reviewer_isolated(task)
            )

    def test_task0068_replacement_is_exact_and_atomic(self) -> None:
        parent = doctor.yaml_at_commit(
            doctor.TASK_0068_PLANNING_REPAIR_PARENT_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        child = doctor.yaml_at_commit(
            doctor.TASK_0068_PLANNING_REPAIR_COMMIT,
            doctor.TASK_BACKLOG_PATH,
        )
        self.assertTrue(doctor.task0068_planning_repair_projection(parent, child))
        audit = Audit()
        validate_backlog_history_edge(
            audit,
            parent,
            child,
            "TASK-0068 fixed parent edge",
            allow_task0068_repair=True,
        )
        self.assertEqual([], audit.errors)
        unauthorized = Audit()
        validate_backlog_history_edge(unauthorized, parent, child, "unauthorized")
        self.assertTrue(unauthorized.errors)
        self.assertTrue(
            doctor.task0068_planning_repair_authorized(
                doctor.TASK_0068_PLANNING_REPAIR_PARENT_COMMIT,
                doctor.TASK_0068_PLANNING_REPAIR_COMMIT,
            )
        )
        task = discover_tasks()["TASK-0068"]
        self.assertTrue(doctor.task0068_repair_approvals_are_exact(task))
        wrong_authority = copy.deepcopy(task)
        wrong_authority["humanApprovals"][1]["scope"] = "wrong"
        self.assertFalse(
            doctor.task0068_repair_approvals_are_exact(wrong_authority)
        )
        expanded = copy.deepcopy(child)
        expanded["tasks"]["TASK-0056"]["objective"] = "expanded"
        self.assertFalse(
            doctor.task0068_planning_repair_projection(parent, expanded)
        )

    def test_task0068_terminal_missing_reviewer_isolation_is_exact_and_fail_closed(
        self,
    ) -> None:
        task = discover_tasks()["TASK-0068"]
        self.assertTrue(doctor.task0068_terminal_missing_reviewer_isolated(task))
        wrong_task = copy.deepcopy(task)
        wrong_task["baseCommit"] = "0" * 40
        self.assertFalse(
            doctor.task0068_terminal_missing_reviewer_isolated(wrong_task)
        )
        with patch.object(
            doctor,
            ...
            self.assertFalse(
                doctor.task0068_terminal_missing_reviewer_isolated(task)
            )
        with patch.dict(
            doctor.TASK_0068_TERMINAL_ARTIFACT_SHA256,
            {"docs/handoffs/TASK-0068.json": "0" * 64},
        ):
            self.assertFalse(
                doctor.task0068_terminal_missing_reviewer_isolated(task)
            )

    def test_task0069_replacement_is_exact_and_atomic(self) -> None:
        child, _, _, _ = self.load_inputs()
        parent = copy.deepcopy(child)
        parent["tasks"]["TASK-0055"]["dependencies"] = ["TASK-0068"]
        self.assertTrue(doctor.task0069_planning_repair_projection(parent, child))
        audit = Audit()
        validate_backlog_history_edge(
            audit,
            parent,
            child,
            "TASK-0069 fixed parent edge",
            allow_task0069_repair=True,
        )
        self.assertEqual([], audit.errors)
        unauthorized = Audit()
        validate_backlog_history_edge(unauthorized, parent, child, "unauthorized")
        self.assertTrue(unauthorized.errors)
        task = discover_tasks()["TASK-0069"]
        self.assertTrue(doctor.task0069_repair_approvals_are_exact(task))
        wrong_remote = copy.deepcopy(task)
        wrong_remote["humanApprovals"][1]["evidence"] = wrong_remote[
            "humanApprovals"
        ][1]["evidence"].replace("UNKNOWN_NOT_RUN", "PASS")
        self.assertFalse(doctor.task0069_repair_approvals_are_exact(wrong_remote))
        authorized_edges = []
        commits = subprocess.check_output(
            ["git", "log", "--format=%H", "--", doctor.TASK_BACKLOG_PATH],
            cwd=ROOT,
            text=True,
        ).splitlines()
        for commit in commits:
            ...
            if len(graph) == 2 and doctor.task0069_planning_repair_authorized(
                graph[1],
                graph[0],
            ):
                authorized_edges.append((graph[1], graph[0]))
    ...
    def test_backlog_projection_exposes_idle_order_and_repository_blockers(self) -> None:
        ...
        self.assertIn(
            "REPOSITORY_NOT_IDLE",
            active_projection["frontierBlockers"],
        )
        self.assertIn(
            "REPOSITORY_NOT_IDLE",
