...
--    租户隔离读路径。
--  - 全仓无 create-snapshot SECURITY DEFINER 函数：retired authorization snapshot store 自述
...
--    legacy runtime 侧 DataCategory 枚举约束（SD 层 text[] 无注入面，元素不在此校验）。
--  - 无新角色、无新表/列/约束/状态；不 REVOKE 任何既有授权；V1-V25 frozen（migration history
--    checksum 安全）。
