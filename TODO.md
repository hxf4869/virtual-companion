...
- [ ] B0-05 演练修复提交：供应商故障演练发现的 pgjdbc「RETURNS void 函数用
      update() 调用抛 a result was returned」缺陷（OwnerContext /
      IdentityAccountRepository 工作区已有修改，含测试后提交）+ 演练记录
      归档 `docs/beta-readiness/records/`（Owner 建立）。

### R43 安全审核覆盖扩充（Beta 前置）

- [ ] SAFETY-RULES-2 确定性规则扩充 §20.10/§20.11 最低覆盖：输出侧补
...
### R45 Alpha 门槛测量最小入口（§26.3 / §26.7）

- [ ] MEASURE 规模化测量脚本：协议仿真（≥10,000 次无「未持久化即
      completed」）、越权规模化、取消后迟到 Token 注入（≥500 次）、
      故障注入统计；可重复执行并出可归档报告。记忆人工标注评测集
      （§26.4，≥200 条候选）是否投入见「待决」。
...
- [ ] D0 轻量产品发现：Beta 前小规模访谈/原型验证（提纲与问卷可由
      Agent 代拟；§7.2/§24.1，仓库从未执行）。
...
- 真实 moderation / embedding 供应商选型与启用时点（Beta 启动时；R43 先扩
  确定性规则，R49 预留接线位）。
- 记忆人工标注评测集（§26.4，≥200 条候选）是否投入人力标注。
- 聊天正文/高敏记忆应用层静态加密时点（§16.5；默认公开版前完成，若 PIA
  审定要求提前则上移到 Beta 前）。
...
> 分别纳入 R33/R34 与 R36。
> 2026-08-19 差距复审的四个方向默认值（Owner 未另行拍板时按此执行）：
> 安全分类先扩确定性规则；补最小测量入口（记忆标注集暂缓）；数据保留
> 版本化先于应用层静态加密；D0 产品发现轻量补做、不阻塞代码轮。
