...
## 当前里程碑（2026-08-19 第三十一轮起）：需求文档差距收尾（R31-R41）
...
- [ ] CHAT-WIPE 全部聊天删除：预览将清除的会话/消息数量 + 两步确认 + 取消
      in-flight work item，保留账号与角色（FR-DATA-003 六粒度的最后一项）。
- [ ] MEM-SUPPRESS 删除防重学：删除记忆时对来源消息建立「禁止再次提取」最小
...
- [ ] SAFETY-QUEUE：R3/R4 高风险人工队列 + admin 只读队列页 + 危机资源现实
      求助提示；处置动作仍人工（§20.5、FR-RES-004）。
- [ ] NL-EXIT：自然语言退出/求助意图确定性识别——立即停止新一轮生成、取消
      在途流式、最多一条简短确认、可审计退出事件（§21.3.4）。

### R35 Beta 运行边界与邀请

- [ ] INVITE 邀请码开通：ADMIN 生成邀请码、凭码开通测试账号（§7.4；
      Alpha 默认关闭）。
- [ ] SVC-WINDOW 服务时段/单日活跃上限/停服开关运行时强制与透明提示（消费
...
- [ ] CONV-SUMMARY L2 会话摘要：覆盖起止 ID/摘要模型与 Prompt 版本/置信度/
      上一版本；低质模型不覆盖高质摘要；补 FR-CHAT-004 会话摘要引用检查（§11.18）。

### R39 Beta 管理端只读页

- [ ] ADMIN-BETA：安全事件中心、举报申诉队列、年龄申诉、导出/删除任务、
      记忆异常抽样只读页（§8.2 管理端；复用 R31-R34 数据，处置动作仍人工）。

### R40 紧急联系人

- [ ] EMERGENCY-CONTACT：验证/变更/撤回/加密存储与最小流程；不自动联系未
      验证联系人（§20.14；紧急联系人同意类型已存在）。

### R41 低敏记忆自动保存（确定性规则版）

- [ ] MEM-AUTO-SAVE：固定低敏类别白名单 + 确定性规则自动保存、可随时撤销、
      界面明示哪些条目是自动保存（§7.4；模型置信度判定留待真实 provider 后）。

### 代码轮完成后：B0 材料起草（Agent 代起草，人工项留 Owner）

- R3/R4 人工处置 Runbook、紧急联系人验证流程、真实用户协议/隐私/测试告知
